...
mod tests {
  ...
  fn collect_styles_keeps_only_whitelisted_props() {
    ...
    assert_eq!(
      out.get("background-color").map(String::as_str),
      Some("#0f766e")
    );
  ...
  fn to_patch_parses_descendant_rules() {
    ...
    let patch = to_patch(v, false);
    assert_eq!(
      patch.styles.get("border-radius").map(String::as_str),
      Some("16px")
    );
    assert_eq!(patch.rules.len(), 2);
    assert_eq!(patch.rules[0].selector, "h1");
    assert_eq!(
      patch.rules[0].styles.get("font-size").map(String::as_str),
      Some("32px")
    );
  ...
  fn chart_prompt_includes_capability_menu_and_type_hint() {
    ...
    let p = build_restyle_prompt(
      "add a target line at 100 and format the axis as currency",
      &ctx,
    );
